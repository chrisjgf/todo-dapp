...
const Main: React.FC = () => {
  ...
  const addTask = async () => {
    const tx = await instance?.createTask("Hello, world", { gasLimit: 750000 });
    console.warn(tx);
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      const mappedTasks: Task[] = tasks.map(t => ({ title: t[0], completed: t[1] }));
  ...
  return (
    <>
      <p onClick={handlePress}>Connect account</p>
      <p>{account}</p>
      <p onClick={addTask}>Add task</p>
      {/* <p onClick={(}>Get task count</p> */}
      {!!tasks && tasks.map((task, i) =>
        <p key={`${i}-${task.title}`}>{task.title}</p>
      )}
    </>
  )
};
